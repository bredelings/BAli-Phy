...
#mycolors <- reverse(mycolors)
mycolors <- c("yellow","green3","blue","red","black")
normal <- 118
levelnames <- c("<= 0.25", "0.26 - 0.50", "0.51 - 0.75", "0.75 - 0.95", ">= 0.96")
#mycolors <- gray((ncolors-1):0 / ncolors)
minmax <- c( c(0,0.25), c(0.26,0.50), c(0.51,0.75), c(0.76,0.95), c(0.96,1) )
mins <- c(0.00, 0.26, 0.51, 0.76, 0.96)
maxs <- c(0.25, 0.50, 0.75, 0.95, 1.00)
tnames <- c("A. salina","D. mobilis", "H. morrhuae", "B. subtilis")
...
# Need to rewrite so colors are painted in ascending order....  
...
plotsegs <- function(xa0, ya0, xa1, ya1, weight, lab1, lab2) {
	...
	if( mymax == 0 ) {
		...
		plot(xa0,ya0,type="n",xlim=c(0,mymax),ylim=c(0,mymax),
