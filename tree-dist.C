...
tree_sample::tree_sample(std::istream& file,const vector<string>& remove) 
{
  ...
  cerr<<"# Loaded "<<trees.size()<<" trees"<<endl;
  ...
  cerr<<"#   There were "<<topologies.size()<<" topologies."<<endl;
...
double branch_distance(const SequenceTree& T1, const SequenceTree& T2) {
  ...
  vector< valarray<bool> > part1(T1.n_branches());

  vector<double> d2(T2.n_branches());
  vector< valarray<bool> > part2(T2.n_branches());
...
double topology_distance(const SequenceTree& T1, const SequenceTree& T2) {
  ...
  for(int b=0;b<T1A.branches();b++)
    T1A.branch(b).length() = 1.0;

  SequenceTree T2A = T2;
  for(int b=0;b<T2A.branches();b++)
    T2A.branch(b).length() = 1.0;
