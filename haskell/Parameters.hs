...
foreign import bpcall "Modifiables:maybe_modifiable_structure" maybe_modifiable_structure :: () -> ()

foreign import bpcall "Modifiables:modifiable" modifiable :: () -> ()

foreign import bpcall "Modifiables:register_prior" builtin_register_prior :: () -> () -> ()
register_prior event prob = IOAction (\s -> (s+1, builtin_register_prior event prob))

foreign import bpcall "Modifiables:register_likelihood" builtin_register_likelihood :: () -> () -> ()
register_likelihood event prob = IOAction (\s -> (s+1, builtin_register_likelihood event prob))


foreign import bpcall "Modifiables:register_in_edge" builtin_register_in_edge :: () -> () -> () -> ()
register_in_edge var dist role = IOAction (\s -> (s+1,builtin_register_in_edge var dist (list_to_string role)))

foreign import bpcall "Modifiables:register_out_edge" builtin_register_out_edge :: () -> () -> ()
register_out_edge dist var = IOAction (\s -> (s+1, builtin_register_out_edge dist var `seq` var))

foreign import bpcall "Modifiables:register_dist_property" builtin_register_dist_property :: () -> () -> () -> ()
...
foreign import bpcall "Modifiables:register_dist" builtin_register_dist :: () -> () -> ()
