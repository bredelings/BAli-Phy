...
foreign import bpcall "MCMC:gibbs_sample_categorical" builtin_gibbs_sample_categorical :: Int -> Int -> Int -> Int -> EPair Int ()
gibbs_sample_categorical x n c = IOAction (pair_from_c . builtin_gibbs_sample_categorical x n c)

foreign import bpcall "MCMC:discrete_uniform_avoid_mh" builtin_discrete_uniform_avoid_mh :: Int -> Int -> Int -> Int -> Int -> EPair Int ()
...
foreign import bpcall "MCMC:slice_sample_integer_random_variable" builtin_slice_sample_integer_random_variable :: Int -> BuiltinBounds -> ContextIndex -> Int -> EPair Int ()
...
-- This is "unsafe" because it doesn't update alignments
foreign import bpcall "MCMC:FNPR_unsafe" builtin_fnpr_unsafe_proposal :: TreeImp -> Int -> ContextIndex -> Int -> EPair Int LogDouble
...
foreign import bpcall "MCMC:walk_tree_sample_alignments" builtin_walk_tree_sample_alignments :: TreeImp -> ContextIndex -> Int -> EPair Int ()
walk_tree_sample_alignments tree c = IOAction (pair_from_c . builtin_walk_tree_sample_alignments tree c)

foreign import bpcall "MCMC:realign_from_tips" builtin_realign_from_tips :: TreeImp -> ContextIndex -> Int -> EPair Int ()
realign_from_tips tree c = IOAction (pair_from_c . builtin_realign_from_tips tree c)

foreign import bpcall "MCMC:walk_tree_sample_NNI" builtin_walk_tree_sample_NNI :: TreeImp -> ContextIndex -> Int -> EPair Int ()
walk_tree_sample_NNI tree c = IOAction (pair_from_c . builtin_walk_tree_sample_NNI tree c)

foreign import bpcall "MCMC:walk_tree_sample_NNI_and_A" builtin_walk_tree_sample_NNI_and_A :: TreeImp -> ContextIndex -> Int -> EPair Int ()
walk_tree_sample_NNI_and_A tree c = IOAction (pair_from_c . builtin_walk_tree_sample_NNI_and_A tree c)

foreign import bpcall "MCMC:walk_tree_sample_NNI_and_branch_lengths" builtin_walk_tree_sample_NNI_and_branch_lengths :: TreeImp -> ContextIndex -> Int -> EPair Int ()
...
foreign import bpcall "MCMC:walk_tree_sample_branch_lengths" builtin_walk_tree_sample_branch_lengths :: TreeImp -> ContextIndex -> Int -> EPair Int ()
walk_tree_sample_branch_lengths tree c = IOAction (pair_from_c . builtin_walk_tree_sample_branch_lengths tree c)

foreign import bpcall "MCMC:sample_SPR_all" builtin_sample_SPR_all :: TreeImp -> ContextIndex -> Int -> EPair Int ()
sample_SPR_all tree c = IOAction (pair_from_c . builtin_sample_SPR_all tree c)

foreign import bpcall "MCMC:sample_SPR_nodes" builtin_sample_SPR_nodes :: TreeImp -> ContextIndex -> Int -> EPair Int ()
sample_SPR_nodes tree c = IOAction (pair_from_c . builtin_sample_SPR_nodes tree c)

foreign import bpcall "MCMC:sample_SPR_flat" builtin_sample_SPR_flat :: TreeImp -> ContextIndex -> Int -> EPair Int ()
sample_SPR_flat tree c = IOAction (pair_from_c . builtin_sample_SPR_flat tree c)

foreign import bpcall "MCMC:copy_context" builtin_copy_context :: ContextIndex -> Int -> EPair Int ContextIndex
copy_context c = IOAction (pair_from_c . builtin_copy_context c)

foreign import bpcall "MCMC:release_context" builtin_release_context :: ContextIndex -> Int -> EPair Int ContextIndex
release_context c = IOAction (pair_from_c . builtin_release_context c)

foreign import bpcall "MCMC:switch_to_context" builtin_switch_to_context :: ContextIndex -> ContextIndex -> Int -> EPair Int ()
switch_to_context c1 c2  = IOAction (pair_from_c . builtin_switch_to_context c1 c2)

foreign import bpcall "MCMC:accept_MH" builtin_accept_MH :: ContextIndex -> ContextIndex -> LogDouble -> Int -> EPair Int Bool
