...
foreign import bpcall "Bits:sized_bitvector_from_list" bitVectorFromListNative :: Int -> [Bool] -> CBitVector
...
instance Unbox Bit where
    ...
    basicFromListN count values = V_Bit count (bitVectorFromListNative count (List.map unBit values))
...
-- Wrap a native result whose exact output length was already established by
-- the Haskell operation that requested it.
bitVectorFromNativeWithLength :: Int -> CBitVector -> Vector Bit
bitVectorFromNativeWithLength = V_Bit
