...
independent dists = Distribution "independent" (independent_densities dists) (no_quantile "independent") (sequence dists) (ListRange (map distRange dists))
...
independent_on dists_pairs = Distribution "independent_on" (independent_densities_on dists_pairs) (no_quantile "independent_on") (sample_independent_on dists_pairs) Nothing


-- If we could make 'independent' work, we could then define `iid n dist = do xs <- independent (repeat dist) ; return $ take n xs`
--  or something like that.
iid n dist = Distribution iid_name (independent_densities (replicate n dist)) (no_quantile "iid") iid_sample (ListRange $ take n $ repeat $ distRange dist) where
    iid_name = "iid "++(dist_name dist)
    iid_sample = do xs <- SamplingRate (1.0/sqrt (intToDouble n)) $ sequence (repeat dist)
                    return $ take n xs

iid_on keys dist = independent_on (zip keys (repeat dist))
