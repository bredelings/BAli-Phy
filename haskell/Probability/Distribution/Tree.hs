...
force_tree tree@(Tree nodes branches n_nodes) = force_nodes `seq` force_branches where
    ...
    force_nodes    = force_struct $ listArray' [ force_struct $ edgesArrayOutOfNode tree node | node <- xrange 0 n_nodes ]
...
modifiable_cayley_tree modf tree = Tree (listArray' nodes) (listArray' branches) n_nodes where
    ...
    degree node | n_leaves == 1   = 0
                | node < n_leaves = 1
                | otherwise       = 3
    nodes    = [ arrayMapN (degree node) modf (edgesArrayOutOfNode tree node) | node <- xrange 0 n_nodes ]
