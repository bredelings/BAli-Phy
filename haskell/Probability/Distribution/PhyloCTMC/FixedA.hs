...
instance (HasLabels t, HasBranchLengths t, IsTree t, SimpleTransitionModel t s) => HasAnnotatedPdf (PhyloCTMC t Int s EquilibriumReversible) where
...
instance (IsTree t, HasRoot (Rooted t), HasLabels t, HasBranchLengths (Rooted t), SimpleTransitionModel (Rooted t) s) => IOSampleable (PhyloCTMC t Int s EquilibriumReversible) where
...
instance (IsTree t, HasRoot (Rooted t), HasLabels t, HasBranchLengths t, HasBranchLengths (Rooted t), SimpleTransitionModel (Rooted t) s, SimpleTransitionModel t s) => Sampleable (PhyloCTMC t Int s EquilibriumReversible) where
...
-- TODO: make this work on forests!                  -
instance (HasLabels t, HasRoot t, HasBranchLengths t, IsTree t, SimpleTransitionModel t s) => HasAnnotatedPdf (PhyloCTMC t Int s EquilibriumNonReversible) where
...
instance (IsTree t, HasRoot t, HasLabels t, HasBranchLengths t, SimpleTransitionModel t s) => IOSampleable (PhyloCTMC t Int s EquilibriumNonReversible) where
    sampleIO (PhyloCTMC tree rootLength smodel scale) = do
      let alphabet = getAlphabet smodel
          smap = stateLetters smodel
...
instance (IsTree t, HasRoot t, HasLabels t, HasBranchLengths t, HasBranchLengths t, SimpleTransitionModel t s) => Sampleable (PhyloCTMC t Int s EquilibriumNonReversible) where
    sample dist = RanDistribution2 dist do_nothing
...
-- TODO: make this work on forests!                  -
instance (HasLabels t, HasRoot t, HasBranchLengths t, IsTree t, SimpleTransitionModel t s) => HasAnnotatedPdf (PhyloCTMC t Int s NonEquilibrium) where
...
instance (IsTree t, HasRoot t, HasLabels t, HasBranchLengths t, SimpleTransitionModel t s) => IOSampleable (PhyloCTMC t Int s NonEquilibrium) where
    sampleIO (PhyloCTMC tree rootLength smodel scale) = do
      let alphabet = getAlphabet smodel
          smap = stateLetters smodel
...
instance (IsTree t, HasRoot t, HasLabels t, HasBranchLengths t, HasBranchLengths t, SimpleTransitionModel t s) => Sampleable (PhyloCTMC t Int s NonEquilibrium) where
    sample dist = RanDistribution2 dist do_nothing
