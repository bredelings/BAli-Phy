...
normal_effect x = x `seq` bnds `seq` add_move (\c -> slice_sample_real_random_variable x bnds c) where bnds = c_range normal_bounds
...
normal m s = Distribution (make_densities $ normal_density m s) (normal_quantile m s) (sample_normal m s) normal_bounds
