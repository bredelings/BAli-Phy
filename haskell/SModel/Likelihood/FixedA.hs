...
foreign import trcall "LikelihoodSEV:simpleSequenceLikelihoods" simpleSequenceLikelihoodsRaw :: Alphabet -> AmbiguityDatabase -> U.Vector Int -> Int -> EPair (EVector Int) CBitVector -> CondLikes

simpleSequenceLikelihoods alpha ambiguities smap nModels (sequence, mask) =
    simpleSequenceLikelihoodsRaw alpha ambiguities smap nModels
...
simpleNodeCLVs :: Alphabet -> AmbiguityDatabase -> U.Vector Int -> Int -> IntMap (Maybe (U.Vector Int, U.Vector Bit)) -> IntMap (Maybe CondLikes)
