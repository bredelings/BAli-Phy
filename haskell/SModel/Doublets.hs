...
x2x2 a m1 m2 = reversible_markov a smap q pi where
    smap = simple_smap a
    q = singlet_to_doublet_rates a (get_q m1) (get_q m2)
    pi = f2x4_frequencies_builtin a (get_pi m1) (get_pi m2)
...
rna_editting a nuc_model edits = reversible_markov a smap q pi
    where smap = simple_smap a
          nuc_a = getNucleotides a
          q_nuc = get_q nuc_model
          pi_nuc = get_pi nuc_model
