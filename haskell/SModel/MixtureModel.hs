...
instance (CTMC m, HasAlphabet m, SimpleSModel m) => SimpleSModel (Discrete m) where
...
instance (HasBranchLengths t, CTMC m, RateModel m, SimpleTransitionModel t m) => SimpleTransitionModel t (Discrete m) where
