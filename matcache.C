...
void MatCache::setlength(int b,double l,tree& T,const substitution::MultiRateModel& SModel) {
  assert(l >= 0);
...
void MatCache::recalc(const tree& T,const substitution::MultiRateModel& SModel) {
  for(int b=0;b<T.branches();b++)
    for(int r=0;r<SModel.nrates();r++)
      transition_P_[r][b] = SModel.transition_p(T.branch(b).length(),r);
}

MatCache::MatCache(const tree& T,const substitution::MultiRateModel& SM) 
