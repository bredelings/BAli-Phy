...
extern "C" closure builtin_function_getEigensystemRaw(OperationArgs& Args)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if (not (pi[i]*n > 1.0e-13)) return closure::object_value(EMaybe());

        sqrt_pi[i] = sqrt(pi[i]);
        inverse_sqrt_pi[i] = 1.0/sqrt_pi[i];

        // fail if we see Inf, -Inf or Nan.
        if (not std::isfinite(pi[i]) or not std::isfinite(inverse_sqrt_pi[i])) return closure::object_value(EMaybe());
    }
...
#endif
            // fail if we see Inf, -Inf or Nan.
            if (not std::isfinite(S(i,j))) return closure::object_value(EMaybe());
	}
    ...
    if (eigensolver->info() != Eigen::Success)
        return closure::object_value(EMaybe());
    else if (std::abs(eigensolver->eigenvalues().maxCoeff()) > 1.0e-9)
    {
        ...
        return closure::object_value(EMaybe(eigensolver));
}
...
extern "C" closure builtin_function_lExpRaw(OperationArgs& Args)
{
    ...
    if (error > 1.0e-9)
        ...
        return closure::object_value(EMaybe(Mptr));
}
