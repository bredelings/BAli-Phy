...
extern "C" closure builtin_function_registerTransitionKernelRaw(OperationArgs& Args)
{
    ...
    auto E = Runtime::App(Runtime::ConstructorApp(constructor("Effect.TransitionKernel",2)),
                          {Runtime::Double(rate), Runtime::IndexVar(0)});
...
extern "C" closure builtin_function_registerLogger(OperationArgs& Args)
{
    ...
    auto E = Runtime::App(Runtime::ConstructorApp(constructor("Effect.Logger",1)),
                          {Runtime::IndexVar(0)});
...
log_double_t get_multiplier(context_ref& C1, const vector<int>& I_regs)
{
    ...
    for(int r : I_regs)
    {
        ...
        context C2 = C1;
        C2.set_reg_value(r, closure(1-i));
...
extern "C" closure builtin_function_sum_out_coals(OperationArgs& Args)
{
    ...
    context C2 = C1;
    C2.set_reg_value(t_reg, closure(t2));
...
extern "C" closure builtin_function_gibbsSampleCategoricalRaw(OperationArgs& Args)
{
	...
	if (i != x1)
	{
            ...
            C2.set_reg_value(*x_mod_reg, closure(i));
    ...
    if (x2 == x1)
        ...
        C1.set_reg_value(*x_mod_reg, closure(x2));
    ...
    return [=](context_ref& C)
           {
               ...
               // 4. Set the new value
               C.set_reg_value(*x_mod_reg, closure(x2));
