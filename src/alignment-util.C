...
void graph_alignment::add_row_character_to_column(int row, int column)
{
  ...
  assert( get_column(row,index) == new_column );
}
...
ublas::matrix<int> construct(const Tree& T, const vector<pairwise_alignment_t>& A, const vector<int>& seqlength)
{
  graph_alignment a(T.n_nodes());

  // Add the first sequence
  for(int i=0;i<seqlength[0];i++)
