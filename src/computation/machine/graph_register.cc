...
void reg_heap::_register_effect_at_reg(int r, int s)
{
    ...
    {
        double rate = closure_at(r).slot(0).as_double();
...
prob_ratios_t reg_heap::probability_ratios(int c1, int c2)
{
    ...
    auto register_dist_handler = [&](int r, int)
    {
        ...
        int observation = closure_at(r).slot(1).as_int();
        if (not observation)
...
void reg_heap::register_in_edge(int r, int /* s */)
{
    ...
    int r_to_dist   = closure_at(r).slot(1).as_int();
    string arg_name = closure_at(r).slot(2).as_string();
...
//    int observation = closure_at(r).get_code().as_<...>()[1].as_string();
    string name = closure_at(r).slot(2).as_string();
...
{
    int r_from_dist = closure_at(r).slot(0).as_int();
    string property =  closure_at(r).slot(1).as_string();
