...
prob_ratios_t reg_heap::probability_ratios(int c1, int c2)
{
    ...
    auto register_dist_handler = [&](int r, int)
    {
        ...
        int observation = closure_at(r).runtime_slot(1).as_int();
        if (not observation)
...
void reg_heap::register_in_edge(int r, int /* s */)
{
    ...
    int r_to_dist   = closure_at(r).runtime_slot(1).as_int();
    string arg_name = closure_at(r).runtime_slot(2).as_string();
...
{
    int r_from_dist = closure_at(r).runtime_reg_for_slot(0);
    int r_to_var    = closure_at(r).runtime_reg_for_slot(1);
...
{
    int r_from_dist = closure_at(r).runtime_slot(0).as_int();
