...
std::pair<Hs::LType,std::vector<bool>> pop_constructor_signature_strictness(Hs::LType ltype);
...
struct TypeFamilyInstanceCheck
{
    FamilyInstanceHead head;
    Type rhs;
};

enum class Apartness
{
    Unifiable,
    SurelyApart,
    MaybeApart
...
public:
    ...
    const DataFamInfo* info_for_data_fam(const std::string& fname) const;
    ...
    public:
        NoteScope(TypeChecker& tc, const Note& n):type_checker(tc) {type_checker.push_note(n);}
        ~NoteScope() noexcept { if (active) type_checker.pop_note(); }

        NoteScope(const NoteScope&) = delete;
        NoteScope& operator=(const NoteScope&) = delete;

        NoteScope(NoteScope&& other) noexcept
            :type_checker(other.type_checker),
             active(other.active)
        {
            other.active = false;
        }

        NoteScope& operator=(NoteScope&&) = delete;
    };

    [[nodiscard]] NoteScope note_scope(const Note& n) {return NoteScope(*this, n);}
    ...
    class SourceSpanScope
    {
        TypeChecker& type_checker;
        bool active = false;

    public:
        SourceSpanScope(TypeChecker& tc, const yy::location& loc)
            :type_checker(tc),
             active(true)
        {
            type_checker.push_source_span(loc);
        }

        SourceSpanScope(TypeChecker& tc, const std::optional<yy::location>& loc)
            :type_checker(tc),
             active(loc.has_value())
        {
            if (active) type_checker.push_source_span(*loc);
        }

        ~SourceSpanScope() noexcept { if (active) type_checker.pop_source_span(); }

        SourceSpanScope(const SourceSpanScope&) = delete;
        SourceSpanScope& operator=(const SourceSpanScope&) = delete;

        SourceSpanScope(SourceSpanScope&& other) noexcept
            :type_checker(other.type_checker),
             active(other.active)
        {
            other.active = false;
        ...
        SourceSpanScope& operator=(SourceSpanScope&&) = delete;
    };

    [[nodiscard]] SourceSpanScope source_span_scope(const yy::location& loc) {return SourceSpanScope(*this, loc);}
    [[nodiscard]] SourceSpanScope source_span_scope(const std::optional<yy::location>& loc) {return SourceSpanScope(*this, loc);}
    ...
    bool type_con_is_data_fam(const TypeCon& tc) const;
    ...
    std::optional<std::tuple<TypeCon,std::vector<Type>>> is_data_fam_app(const Type& t) const;
    ...
    std::optional<std::pair<Type, Type>> open_forall_pair(const ForallType& forall1, const ForallType& forall2);
    ...
    void classify_record_selectors();
    void add_record_selectors_to_env();
    Hs::Decls typecheck_generated_record_selectors();
    ...
    TypeFamilyInstanceCheck check_type_instance(const Hs::LType&, const Hs::LType&);
    ...
    bool try_fill_meta_type_var(const MetaTypeVar& tv, const Type& type) const;
    ...
    std::optional<bsubstitution_t> maybe_unify(const std::vector<Type>& t1, const std::vector<Type>& t2) const;

    std::optional<bsubstitution_t> maybe_match(const Type& t1, const Type& t2) const;
    std::optional<bsubstitution_t> maybe_match(const std::vector<Type>& t1, const std::vector<Type>& t2) const;
    ...
    Apartness apartness(const Type& t1, const Type& t2) const;
    Apartness apartness(const std::vector<Type>& t1, const std::vector<Type>& t2) const;
    // end pure unifier.
    ...
    TypeFamEqnInfo freshen(TypeFamEqnInfo);

    std::tuple<std::vector<MetaTypeVar>, LIE, Type> instantiate(const ConstraintOrigin& origin, const Type& t);

    [[nodiscard]] std::tuple<Core::wrapper, Type> instantiate_emit(const ConstraintOrigin& origin, const Type& t);

    [[nodiscard]] Core::wrapper subsumptionCheck(const ConstraintOrigin& origin, const Type& t1, const Expected& e2);

    [[nodiscard]] Core::wrapper subsumptionCheck(const ConstraintOrigin& origin, const Type& t1, const Type& t2);

    [[nodiscard]] Core::wrapper instantiateSigma(const ConstraintOrigin& origin, const Type& t, const Expected& exp_rho);

    [[nodiscard]] std::tuple<Core::wrapper, std::vector<TypeVar>, LIE, Type> skolemize(const Type& t, bool skolem);

    [[nodiscard]] std::tuple<Core::wrapper, std::vector<TypeVar>, LIE, Type> skolemize_and(const Type& t, const tc_action<Type>& nested_action);

    template <typename R>
    [[nodiscard]] std::tuple<Core::wrapper, std::vector<TypeVar>, LIE, Type, R> skolemize_and_result(const Type& t, const tc_function<R, Type>& nested_action);
    ...
    [[nodiscard]] Core::wrapper instPatSigma(const SigmaType& sigma_type, const Expected& exp_sigma_type);
    ...
    [[nodiscard]] Core::wrapper checkSigma(Hs::LExp& e, const SigmaType& sigma_type);
    ...
    void tcRho(Hs::Apply& app_exp, const Expected& E);

    [[nodiscard]] Core::wrapper tcRho(Hs::Lambda&, const Expected& E);

    void tcRho(Hs::Let&, const Expected&);

    void tcRho(Hs::TypedExp& TExp, const Expected&);

    void tcRho(Hs::Case& Case, const Expected&);

    void tcCaseAlts(Hs::Matches& alts, const Type& object_type, const Expected&);
    ...
    void tcRho(Hs::If&, const Expected&);

    void tcRho(Hs::MultiWayIf&, const Expected&);
    ...
    void tcBody(Hs::LExp& body, const Expected&);
    ...
    template <typename A>
    [[nodiscard]] Core::wrapper tcMatchesFunInfer(std::vector<Expected>& arg_types, int arity, const Expected& fun_type, const A& a);

    template <typename A>
    [[nodiscard]] Core::wrapper tcMatchesFunCheck(std::vector<Expected>& arg_types, int arity, Type type, const A& a);

    template <typename A>
    [[nodiscard]] Core::wrapper tcMatchesFun(int arity, const Expected&, const A& a);
    ...
    Core::Decls<> infer_type_for_classes(const Hs::Decls& decls);

    std::tuple<ClassInfo,Core::Decls<>>
    ...
    DataConInfo infer_type_for_data_family_constructor(const Hs::LType& result_type, const std::vector<Hs::LTypeVar>& outer_tvs, const std::vector<Type>& top_constraints, const Hs::ConstructorDecl& constructor);

    DataConInfo infer_type_for_gadt_data_family_constructor(const Type& instance_head, const TypeCon& data_family, const std::vector<Type>& top_constraints, const Hs::GADTConstructorDecl& constructor, bool is_newtype);

    std::vector<Role> infer_roles_for_data_type(const std::vector<TypeVar>& data_tvs, const DataConEnv& constructors) const;
    void apply_source_role_annotations(const Hs::Decls& decls);

    std::pair<DataConEnv,std::optional<Type>> infer_type_for_data_family_instance(const Hs::DataFamilyInstanceDecl& data_inst, const std::optional<std::string>& associated_class = {});
    ...
    std::optional<Type> collect_derived_context(const Type& pred, const std::set<TypeVar>& data_tvs,
                                                std::vector<Type>& active, Context& context);

    void infer_stock_derived_contexts(std::vector<std::pair<Core::Var<>, Hs::InstanceDecl>>& named_instances);
    ...
    std::tuple<Hs::Decl,Core::Var<>> type_check_superclass_entry(
        const Constraint& superclass_constraint, const std::string& class_name,
    ...
    std::tuple<Hs::Decl, Core::Var<>>
    type_check_instance_method(const Hs::Var& method, const Type& method_type, const std::vector<TypeVar>& instance_tvs, const LIE& givens, const Type& inst_head, const substitution_t& subst, const std::map<Hs::Var,Hs::Matches>& method_matches, const ClassInfo& class_info);

    std::tuple<Hs::Decl, Core::Var<>>
    type_check_gnd_instance_method(const Hs::Var& method, const Type& method_type, const std::vector<TypeVar>& instance_tvs, const LIE& givens, const substitution_t& subst, const ClassInfo& class_info, const TypeCon& class_con, const std::vector<Type>& instance_args);
    ...
    void add_type_instance(const TypeFamEqnInfo& equation);

    std::optional<TypeFamEqnInfo> check_type_family_equation(
        const Hs::TypeFamilyInstanceEqn& inst,
        const std::optional<std::string>& associated_class,
        const substitution_t& instance_subst,
        bool closed_branch);

    bool compatible_type_family_equations(const TypeFamEqnInfo& first,
                                           const TypeFamEqnInfo& second);

    bool check_open_type_family_equation(const TypeFamEqnInfo& candidate);

    void check_imported_type_family_consistency();

    void finish_closed_type_family(TypeFamInfo& family_info);

    Type normalize_foreign_type(const Type&);

    std::optional<Type> direct_raw_import_public_type(const Type&);

    std::optional<Hs::ForeignABILayout>
    make_foreign_abi_layout(const Type& public_type, const Type& abi_type);

    void annotate_foreign_declarations(std::vector<Hs::ForeignDecl>&);

    bool check_family_instance_association(const Hs::LTypeCon& family_con,
					   const std::optional<std::string>& family_associated_class,
					   const std::optional<std::string>& instance_associated_class,
					   const std::string& instance_name,
					   const std::string& family_name,
					   bool require_associated_family,
					   bool indent_message);

    FamilyInstanceHead check_family_instance_head(const Hs::LTypeCon& family_con,
						  const std::vector<Hs::LType>& args,
						  const std::optional<std::vector<Hs::LTypeVar>>& forall,
						  const Hs::Context& context);

    void check_associated_family_instance_args(const std::vector<Hs::LType>& hs_args,
					       const std::vector<Type>& args,
					       const std::vector<TypeVar>& family_args,
					       const substitution_t& instance_subst);
    ...
    void check_data_instance(const Hs::DataFamilyInstanceDecl& inst, const std::optional<std::string>& associated_class, const substitution_t& instance_subst);
    ...
    std::optional<Type> unwrap_newtype_type(const Type&) const;

    void get_defaults(const Hs::ModuleDecls& M);

    typechecker_result typecheck_module( Hs::ModuleDecls M );

    TypeChecker(Module& m);
};
...
std::vector<std::vector<Hs::Decl>> find_type_groups(const Hs::Decls& type_decls);
...
std::string class_arg_name(const Type& class_arg);
