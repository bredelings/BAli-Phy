...
#include "computation/haskell/haskell.hh" // for Hs::Var, Type, Hs::Kind, TypeVar, Hs::Context, Hs::MetaTypeVar
...
{
    ID name;
    Hs::DataOrNewtype data_or_newtype = Hs::DataOrNewtype::data;
    std::vector<TypeVar> type_vars;
    Context context;
    ...
    void serialize(Archive& ar)
    {
        ar(name, data_or_newtype, type_vars, context, constructors);
    }
};

struct FunctionalDependency
{
    // Class-parameter indexes remain meaningful after interface serialization.
...
struct ClassInfo
{
    ...
    std::set<TypeCon> associated_data_families;
	...
	ar(name, type_vars, context, functional_dependencies, members, superclass_extractors, default_methods, fields, associated_type_families, associated_data_families);
    }
};

enum class TypeFamilyFlavor
{
    Open,
    Closed
};

using TypeFamBranchIndex = unsigned;

struct ClosedTypeFamEqnInfo
{
    TypeFamEqnInfo equation;
    std::vector<TypeFamBranchIndex> incompatible_predecessors;

    template <class Archive>
    void serialize(Archive& ar)
    {
	ar(equation, incompatible_predecessors);
    }
};

struct ClosedTypeFamInfo
{
    std::vector<ClosedTypeFamEqnInfo> equations;
...
struct TypeFamInfo
{
    ...
    TypeFamilyFlavor flavor = TypeFamilyFlavor::Open;
    std::optional<ClosedTypeFamInfo> closed_family;

    int arity() const {return args.size();}
    bool is_open() const {return flavor == TypeFamilyFlavor::Open;}
    bool is_closed() const {return flavor == TypeFamilyFlavor::Closed;}

    template <class Archive>
    void serialize(Archive& ar)
    {
	ar(args, result_kind, associated_class, flavor, closed_family);
    }
