...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // impspec
      char dummy12[sizeof (Hs::ImpSpec)];

      // kind
      char dummy13[sizeof (Hs::Kind)];

      // export
      char dummy14[sizeof (Hs::LExport)];

      // importdecl
      char dummy15[sizeof (Hs::LImpDecl)];
      ...
      // inst_type
      // constr_stuff
      char dummy16[sizeof (Hs::LType)];

      // tv_bndr
      // tv_bndr_no_braces
      char dummy17[sizeof (Hs::LTypeVar)];

      // module
      char dummy18[sizeof (Hs::Module)];

      // rhs
      // alt_rhs
      char dummy19[sizeof (Hs::MultiGuardedRHS)];

      // stmtlist
      char dummy20[sizeof (Hs::Stmts)];
      ...
      // fbinds
      // fbinds1
      char dummy25[sizeof (Located<Hs::FieldBindings>)];

      // infixexp
      char dummy26[sizeof (Located<Hs::InfixExp>)];
      ...
      // stmt
      // qual
      char dummy27[sizeof (Located<expression_ref>)];

      // qcname
      // modid
      char dummy28[sizeof (Located<std::string>)];

      // optqualified
      char dummy29[sizeof (bool)];

      // "CHAR"
      // "PRIMCHAR"
      char dummy30[sizeof (char)];

      // "PRIMDOUBLE"
      char dummy31[sizeof (double)];

      // list
      // qop
      // qopm
      // literal
      char dummy32[sizeof (expression_ref)];

      // "PRIMFLOAT"
      char dummy33[sizeof (float)];

      // "PRIMWORD"
      // commas
      char dummy34[sizeof (int)];

      // "INTEGER"
      // "PRIMINTEGER"
      char dummy35[sizeof (integer)];

      // "RATIONAL"
      char dummy36[sizeof (rational)];

      // export_subspec
      char dummy37[sizeof (std::optional<Hs::ExportSubSpec>)];

      // maybeimpspec
      char dummy38[sizeof (std::optional<Hs::ImpSpec>)];

      // opt_kind_sig
      char dummy39[sizeof (std::optional<Hs::Kind>)];

      // opt_tyconsig
      char dummy40[sizeof (std::optional<Hs::LType>)];

      // wherebinds
      char dummy41[sizeof (std::optional<Located<Hs::Binds>>)];

      // where_cls
      // where_inst
      char dummy42[sizeof (std::optional<Located<Hs::Decls>>)];

      // fbind
      char dummy43[sizeof (std::optional<Located<Hs::FieldBinding>>)];

      // opt_datafam_kind_sig
      // opt_tyfam_kind_sig
      // opt_at_kind_inj_sig
      char dummy44[sizeof (std::optional<Located<Hs::Kind>>)];

      // maybeas
      char dummy45[sizeof (std::optional<Located<std::string>>)];

      // prec
      char dummy46[sizeof (std::optional<int>)];

      // overlap_pragma
      char dummy47[sizeof (std::optional<std::string>)];

      // maybeexports
      char dummy48[sizeof (std::optional<std::vector<Hs::LExport>>)];

      // where_type_family
      char dummy49[sizeof (std::optional<std::vector<Hs::TypeFamilyInstanceEqn>>)];

      // tycl_hdr
      char dummy50[sizeof (std::pair<Hs::Context,Hs::LType>)];

      // body
      // body2
      // top
      // top1
      char dummy51[sizeof (std::pair<std::vector<Hs::LImpDecl>, std::optional<Hs::Decls>>)];
      ...
      // datafam_inst_hdr
      char dummy53[sizeof (std::tuple<std::optional<std::vector<Hs::LTypeVar>>,std::optional<Located<Hs::Context>>,Hs::LType>)];

      // fielddecls
      // fielddecls1
      char dummy54[sizeof (std::vector<Hs::FieldDecl>)];

      // gdrhs
      // gdpats
      char dummy55[sizeof (std::vector<Hs::GuardedRHS>)];

      // exportlist
      // exportlist1
      char dummy56[sizeof (std::vector<Hs::LExport>)];

      // importdecls
      // importdecls_semi
      char dummy57[sizeof (std::vector<Hs::LImpDecl>)];

      // sigtypes1
      // btype_no_ops
      // comma_types0
      // comma_types1
      char dummy58[sizeof (std::vector<Hs::LType>)];

      // sks_vars
      char dummy59[sizeof (std::vector<Hs::LTypeCon>)];

      // tv_bndrs
      // forall
      char dummy60[sizeof (std::vector<Hs::LTypeVar>)];

      // sig_vars
      char dummy61[sizeof (std::vector<Hs::LVar>)];

      // ty_fam_inst_eqn_list
      // ty_fam_inst_eqns
      char dummy62[sizeof (std::vector<Hs::TypeFamilyInstanceEqn>)];

      // alts
      // alts1
      char dummy63[sizeof (std::vector<Located<Hs::Alt>>)];
      ...
      // apats1
      // stmts
      char dummy64[sizeof (std::vector<Located<expression_ref>>)];

      // qcnames
      // qcnames1
      // ops
      // con_list
      char dummy65[sizeof (std::vector<Located<std::string>>)];
    };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_standalone_kind_sig = 179,             // standalone_kind_sig
        S_sks_vars = 180,                        // sks_vars
        S_inst_decl = 181,                       // inst_decl
        S_overlap_pragma = 182,                  // overlap_pragma
        S_deriv_strategy_no_via = 183,           // deriv_strategy_no_via
        S_deriv_strategy_via = 184,              // deriv_strategy_via
        S_opt_injective_info = 185,              // opt_injective_info
        S_injectivity_cond = 186,                // injectivity_cond
        S_inj_varids = 187,                      // inj_varids
        S_where_type_family = 188,               // where_type_family
        S_ty_fam_inst_eqn_list = 189,            // ty_fam_inst_eqn_list
        S_ty_fam_inst_eqns = 190,                // ty_fam_inst_eqns
        S_ty_fam_inst_eqn = 191,                 // ty_fam_inst_eqn
        S_at_decl_cls = 192,                     // at_decl_cls
        S_opt_family = 193,                      // opt_family
        S_opt_instance = 194,                    // opt_instance
        S_at_decl_inst = 195,                    // at_decl_inst
        S_data_or_newtype = 196,                 // data_or_newtype
        S_opt_kind_sig = 197,                    // opt_kind_sig
        S_opt_datafam_kind_sig = 198,            // opt_datafam_kind_sig
        S_opt_tyfam_kind_sig = 199,              // opt_tyfam_kind_sig
        S_opt_at_kind_inj_sig = 200,             // opt_at_kind_inj_sig
        S_tycl_hdr = 201,                        // tycl_hdr
        S_datafam_inst_hdr = 202,                // datafam_inst_hdr
        S_capi_ctype = 203,                      // capi_ctype
        S_decl_cls = 204,                        // decl_cls
        S_decls_cls = 205,                       // decls_cls
        S_decllist_cls = 206,                    // decllist_cls
        S_where_cls = 207,                       // where_cls
        S_decl_inst = 208,                       // decl_inst
        S_decls_inst = 209,                      // decls_inst
        S_decllist_inst = 210,                   // decllist_inst
        S_where_inst = 211,                      // where_inst
        S_decls = 212,                           // decls
        S_decllist = 213,                        // decllist
        S_binds = 214,                           // binds
        S_wherebinds = 215,                      // wherebinds
        S_strings = 216,                         // strings
        S_stringlist = 217,                      // stringlist
        S_opt_tyconsig = 218,                    // opt_tyconsig
        S_sigtype = 219,                         // sigtype
        S_sigtypedoc = 220,                      // sigtypedoc
        S_sig_vars = 221,                        // sig_vars
        S_sigtypes1 = 222,                       // sigtypes1
        S_ktype = 223,                           // ktype
        S_ctype = 224,                           // ctype
        S_ctypedoc = 225,                        // ctypedoc
        S_context = 226,                         // context
        S_context_no_ops = 227,                  // context_no_ops
        S_type = 228,                            // type
        S_typedoc = 229,                         // typedoc
        S_btype = 230,                           // btype
        S_infixtype = 231,                       // infixtype
        S_btype_no_ops = 232,                    // btype_no_ops
        S_ftype = 233,                           // ftype
        S_tyarg = 234,                           // tyarg
        S_tyop = 235,                            // tyop
        S_atype_docs = 236,                      // atype_docs
        S_atype = 237,                           // atype
        S_inst_type = 238,                       // inst_type
        S_deriv_types = 239,                     // deriv_types
        S_comma_types0 = 240,                    // comma_types0
        S_comma_types1 = 241,                    // comma_types1
        S_tv_bndrs = 242,                        // tv_bndrs
        S_tv_bndr = 243,                         // tv_bndr
        S_tv_bndr_no_braces = 244,               // tv_bndr_no_braces
        S_kind = 245,                            // kind
        S_gadt_constrlist = 246,                 // gadt_constrlist
        S_gadt_constrs = 247,                    // gadt_constrs
        S_gadt_constr = 248,                     // gadt_constr
        S_constrs = 249,                         // constrs
        S_constrs1 = 250,                        // constrs1
        S_constr = 251,                          // constr
        S_forall = 252,                          // forall
        S_constr_stuff = 253,                    // constr_stuff
        S_fielddecls = 254,                      // fielddecls
        S_fielddecls1 = 255,                     // fielddecls1
        S_fielddecl = 256,                       // fielddecl
        S_maybe_derivings = 257,                 // maybe_derivings
        S_derivings = 258,                       // derivings
        S_deriving = 259,                        // deriving
        S_deriv_clause_types = 260,              // deriv_clause_types
        S_decl_no_th = 261,                      // decl_no_th
        S_decl = 262,                            // decl
        S_rhs = 263,                             // rhs
        S_gdrhs = 264,                           // gdrhs
        S_gdrh = 265,                            // gdrh
        S_sigdecl = 266,                         // sigdecl
        S_activation = 267,                      // activation
        S_explicit_activation = 268,             // explicit_activation
        S_exp = 269,                             // exp
        S_infixexp = 270,                        // infixexp
        S_exp10 = 271,                           // exp10
        S_optSemi = 272,                         // optSemi
        S_fexp = 273,                            // fexp
        S_aexp = 274,                            // aexp
        S_aexp1 = 275,                           // aexp1
        S_aexp2 = 276,                           // aexp2
        S_projection = 277,                      // projection
        S_texp = 278,                            // texp
        S_tup_exprs = 279,                       // tup_exprs
        S_list = 280,                            // list
        S_lexps = 281,                           // lexps
        S_squals = 282,                          // squals
        S_guardquals = 283,                      // guardquals
        S_guardquals1 = 284,                     // guardquals1
        S_altslist = 285,                        // altslist
        S_alts = 286,                            // alts
        S_alts1 = 287,                           // alts1
        S_alt = 288,                             // alt
        S_alt_rhs = 289,                         // alt_rhs
        S_gdpats = 290,                          // gdpats
        S_gdpat = 291,                           // gdpat
        S_pat = 292,                             // pat
        S_bindpat = 293,                         // bindpat
        S_apat = 294,                            // apat
        S_apats1 = 295,                          // apats1
        S_stmtlist = 296,                        // stmtlist
        S_stmts = 297,                           // stmts
        S_stmt = 298,                            // stmt
        S_qual = 299,                            // qual
        S_fbinds = 300,                          // fbinds
        S_fbinds1 = 301,                         // fbinds1
        S_fbind = 302,                           // fbind
        S_fieldToUpdate = 303,                   // fieldToUpdate
        S_qcon = 304,                            // qcon
        S_gen_qcon = 305,                        // gen_qcon
        S_con = 306,                             // con
        S_con_list = 307,                        // con_list
        S_sysdcon_no_list = 308,                 // sysdcon_no_list
        S_sysdcon = 309,                         // sysdcon
        S_conop = 310,                           // conop
        S_qconop = 311,                          // qconop
        S_gtycon = 312,                          // gtycon
        S_ntgtycon = 313,                        // ntgtycon
        S_oqtycon = 314,                         // oqtycon
        S_oqtycon_no_varcon = 315,               // oqtycon_no_varcon
        S_qtyconop = 316,                        // qtyconop
        S_qtycondoc = 317,                       // qtycondoc
        S_qtycon = 318,                          // qtycon
        S_tycon = 319,                           // tycon
        S_qtyconsym = 320,                       // qtyconsym
        S_tyconsym = 321,                        // tyconsym
        S_op = 322,                              // op
        S_varop = 323,                           // varop
        S_qop = 324,                             // qop
        S_qopm = 325,                            // qopm
        S_qvarop = 326,                          // qvarop
        S_qvaropm = 327,                         // qvaropm
        S_tyvar = 328,                           // tyvar
        S_tyvarop = 329,                         // tyvarop
        S_tyvarid = 330,                         // tyvarid
        S_var = 331,                             // var
        S_qvar = 332,                            // qvar
        S_field = 333,                           // field
        S_qvarid = 334,                          // qvarid
        S_varid = 335,                           // varid
        S_qvarsym = 336,                         // qvarsym
        S_qvarsym_no_minus = 337,                // qvarsym_no_minus
        S_qvarsym1 = 338,                        // qvarsym1
        S_varsym = 339,                          // varsym
        S_varsym_no_minus = 340,                 // varsym_no_minus
        S_special_id = 341,                      // special_id
        S_special_sym = 342,                     // special_sym
        S_qconid = 343,                          // qconid
        S_conid = 344,                           // conid
        S_qconsym = 345,                         // qconsym
        S_consym = 346,                          // consym
        S_literal = 347,                         // literal
        S_close = 348,                           // close
        S_modid = 349,                           // modid
        S_commas = 350                           // commas
      };
...
switch (yykind)
  ...
  private:
    ...
    enum
    {
      yylast_ = 5402,     ///< Last index in yytable_.
