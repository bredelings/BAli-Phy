...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // constr
      char dummy1[sizeof (Hs::ConstructorDecl)];

      // constrs
      // constrs1
      char dummy2[sizeof (Hs::ConstructorsDecl)];
      ...
      char dummy6[sizeof (Hs::DerivingStrategy)];

      // list
      // qop
      // qopm
      // literal
      char dummy7[sizeof (Hs::Exp)];
      ...
      // decllist_cls
      // decllist_inst
      char dummy29[sizeof (Located<Hs::Decls>)];

      // fbinds
      // fbinds1
      char dummy30[sizeof (Located<Hs::FieldBindings>)];

      // infixexp
      char dummy31[sizeof (Located<Hs::InfixExp>)];

      // alt
      char dummy32[sizeof (Located<Hs::ParsedAlt>)];

      // role
      char dummy33[sizeof (Located<std::optional<Role>>)];

      // qcname
      // call_conv
      // modid
      char dummy34[sizeof (Located<std::string>)];

      // optqualified
      char dummy35[sizeof (bool)];
      ...
      // "INTEGER"
      // "PRIMINTEGER"
      char dummy40[sizeof (integer)];

      // "RATIONAL"
      char dummy41[sizeof (rational)];

      // export_subspec
      char dummy42[sizeof (std::optional<Hs::ExportSubSpec>)];

      // maybeimpspec
      char dummy43[sizeof (std::optional<Hs::ImpSpec>)];

      // opt_kind_sig
      char dummy44[sizeof (std::optional<Hs::Kind>)];

      // opt_tyconsig
      char dummy45[sizeof (std::optional<Hs::LType>)];

      // wherebinds
      char dummy46[sizeof (std::optional<Located<Hs::Binds>>)];

      // where_cls
      // where_inst
      char dummy47[sizeof (std::optional<Located<Hs::Decls>>)];

      // fbind
      char dummy48[sizeof (std::optional<Located<Hs::FieldBinding>>)];

      // opt_datafam_kind_sig
      // opt_tyfam_kind_sig
      // opt_at_kind_inj_sig
      char dummy49[sizeof (std::optional<Located<Hs::Kind>>)];

      // maybeas
      // opt_class
      char dummy50[sizeof (std::optional<Located<std::string>>)];

      // prec
      char dummy51[sizeof (std::optional<int>)];

      // overlap_pragma
      char dummy52[sizeof (std::optional<std::string>)];

      // maybeexports
      char dummy53[sizeof (std::optional<std::vector<Hs::LExport>>)];

      // where_type_family
      char dummy54[sizeof (std::optional<std::vector<Hs::TypeFamilyInstanceEqn>>)];

      // tycl_hdr
      char dummy55[sizeof (std::pair<Hs::Context,Hs::LType>)];

      // body
      // body2
      // top
      // top1
      char dummy56[sizeof (std::pair<std::vector<Hs::LImpDecl>, std::optional<Hs::Decls>>)];
      ...
      // datafam_inst_hdr
      char dummy58[sizeof (std::tuple<std::optional<std::vector<Hs::LTypeVar>>,std::optional<Located<Hs::Context>>,Hs::LType>)];

      // maybe_derivings
      // derivings
      // deriving
      char dummy59[sizeof (std::vector<Hs::Deriving>)];

      // fielddecls
      // fielddecls1
      char dummy60[sizeof (std::vector<Hs::FieldDecl>)];

      // fds
      // fds1
      char dummy61[sizeof (std::vector<Hs::FunDep>)];
      ...
      // lexps
      // apats1
      char dummy63[sizeof (std::vector<Hs::LExp>)];

      // exportlist
      // exportlist1
      // importlist
      // importlist1
      char dummy64[sizeof (std::vector<Hs::LExport>)];

      // importdecls
      // importdecls_semi
      char dummy65[sizeof (std::vector<Hs::LImpDecl>)];

      // squals
      // guardquals
      // guardquals1
      // stmts
      char dummy66[sizeof (std::vector<Hs::LStmt>)];
      ...
      // comma_types1
      // deriv_clause_types
      char dummy67[sizeof (std::vector<Hs::LType>)];

      // sks_vars
      char dummy68[sizeof (std::vector<Hs::LTypeCon>)];

      // tv_bndrs
      // varids0
      // forall
      char dummy69[sizeof (std::vector<Hs::LTypeVar>)];

      // sig_vars
      char dummy70[sizeof (std::vector<Hs::LVar>)];

      // ty_fam_inst_eqn_list
      // ty_fam_inst_eqns
      char dummy71[sizeof (std::vector<Hs::TypeFamilyInstanceEqn>)];

      // alts
      // alts1
      char dummy72[sizeof (std::vector<Located<Hs::ParsedAlt>>)];

      // maybe_roles
      // roles
      char dummy73[sizeof (std::vector<Located<std::optional<Role>>>)];

      // qcnames
      // qcnames1
      // ops
      // con_list
      char dummy74[sizeof (std::vector<Located<std::string>>)];

      // projection
      char dummy75[sizeof (std::vector<std::string>)];
    };
    ...
    TOK_STOCK = 306,               // "stock"
    TOK_TRCALL = 307,              // "trcall"
    TOK_ECALL = 308,               // "ecall"
    TOK_ANYCLASS = 309,            // "anyclass"
    TOK_VIA = 310,                 // "via"
    TOK_UNIT = 311,                // "unit"
    TOK_SIGNATURE = 312,           // "signature"
    TOK_DEPENDENCY = 313,          // "dependency"
    ...
    TOK_LARROW = 341,              // "<-"
    TOK_RARROW = 342,              // "->"
    TOK_TIGHT_INFIX_AT = 343,      // TIGHT_INFIX_AT
    TOK_PREFIX_AT = 344,           // "@"
    TOK_PREFIX_TILDE = 345,        // PREFIX_TILDE
    TOK_TILDE = 346,               // "~"
    TOK_DARROW = 347,              // "=>"
    TOK_PREFIX_MINUS = 348,        // PREFIX_MINUS
    TOK_MINUS = 349,               // "-"
    TOK_PREFIX_BANG = 350,         // PREFIX_BANG
    TOK_BANG = 351,                // "!"
    TOK_STAR = 352,                // "*"
    TOK_lARROWTAIL = 353,          // "-<"
    TOK_rARROWTAIL = 354,          // ">-"
    TOK_LARROWTAIL = 355,          // "-<<"
    TOK_RARROWTAIL = 356,          // ">>-"
    TOK_TIGHT_INFIX_DOT = 357,     // TIGHT_INFIX_DOT
    TOK_PREFIX_DOT = 358,          // PREFIX_DOT
    TOK_DOT = 359,                 // "."
    TOK_OCURLY = 360,              // "{"
    TOK_CCURLY = 361,              // "}"
    TOK_VOCURLY = 362,             // "vocurly"
    TOK_VCCURLY = 363,             // "vccurly"
    TOK_OBRACK = 364,              // "["
    TOK_CBRACK = 365,              // "]"
    TOK_OPABRACK = 366,            // "[:"
    TOK_CPABRACK = 367,            // ":]"
    ...
    TOK_BACKQUOTE = 376,           // "`"
    TOK_SIMPLEQUOTE = 377,         // "'"
    TOK_INLINE_PRAG = 378,         // "{-# INLINE"
    TOK_VARID = 379,               // "VARID"
    TOK_CONID = 380,               // "CONID"
    TOK_VARSYM = 381,              // "VARSYM"
    TOK_CONSYM = 382,              // "CONSYM"
    TOK_QVARID = 383,              // "QVARID"
    TOK_QCONID = 384,              // "QCONID"
    TOK_QVARSYM = 385,             // "QVARSYM"
    TOK_QCONSYM = 386,             // "QCONSYM"
    TOK_IPDUPVARID = 387,          // "IPDUPVARID"
    TOK_LABELVARID = 388,          // "LABELVARID"
    TOK_CHAR = 389,                // "CHAR"
    TOK_STRING = 390,              // "STRING"
    TOK_INTEGER = 391,             // "INTEGER"
    TOK_RATIONAL = 392,            // "RATIONAL"
    TOK_PRIMCHAR = 393,            // "PRIMCHAR"
    TOK_PRIMSTRING = 394,          // "PRIMSTRING"
    TOK_PRIMINTEGER = 395,         // "PRIMINTEGER"
    TOK_PRINTWORD = 396,           // "PRIMWORD"
    TOK_PRIMFLOAT = 397,           // "PRIMFLOAT"
    TOK_PRIMDOUBLE = 398           // "PRIMDOUBLE"
      };
    ...
    struct symbol_kind
    {
      ...
      {
        YYNTOKENS = 147, ///< Number of tokens.
        S_YYEMPTY = -2,
        ...
        S_TRCALL = 52,                           // "trcall"
        S_ECALL = 53,                            // "ecall"
        S_ANYCLASS = 54,                         // "anyclass"
        S_VIA = 55,                              // "via"
        S_UNIT = 56,                             // "unit"
        S_SIGNATURE = 57,                        // "signature"
        S_DEPENDENCY = 58,                       // "dependency"
        ...
        S_LARROW = 86,                           // "<-"
        S_RARROW = 87,                           // "->"
        S_TIGHT_INFIX_AT = 88,                   // TIGHT_INFIX_AT
        S_PREFIX_AT = 89,                        // "@"
        S_PREFIX_TILDE = 90,                     // PREFIX_TILDE
        S_TILDE = 91,                            // "~"
        S_DARROW = 92,                           // "=>"
        S_PREFIX_MINUS = 93,                     // PREFIX_MINUS
        S_MINUS = 94,                            // "-"
        S_PREFIX_BANG = 95,                      // PREFIX_BANG
        S_BANG = 96,                             // "!"
        S_STAR = 97,                             // "*"
        S_lARROWTAIL = 98,                       // "-<"
        S_rARROWTAIL = 99,                       // ">-"
        S_LARROWTAIL = 100,                      // "-<<"
        S_RARROWTAIL = 101,                      // ">>-"
        S_TIGHT_INFIX_DOT = 102,                 // TIGHT_INFIX_DOT
        S_PREFIX_DOT = 103,                      // PREFIX_DOT
        S_DOT = 104,                             // "."
        S_OCURLY = 105,                          // "{"
        S_CCURLY = 106,                          // "}"
        S_VOCURLY = 107,                         // "vocurly"
        S_VCCURLY = 108,                         // "vccurly"
        S_OBRACK = 109,                          // "["
        S_CBRACK = 110,                          // "]"
        S_OPABRACK = 111,                        // "[:"
        S_CPABRACK = 112,                        // ":]"
        ...
        S_QVARID = 128,                          // "QVARID"
        S_QCONID = 129,                          // "QCONID"
        S_QVARSYM = 130,                         // "QVARSYM"
        S_QCONSYM = 131,                         // "QCONSYM"
        S_IPDUPVARID = 132,                      // "IPDUPVARID"
        S_LABELVARID = 133,                      // "LABELVARID"
        S_CHAR = 134,                            // "CHAR"
        S_STRING = 135,                          // "STRING"
        S_INTEGER = 136,                         // "INTEGER"
        S_RATIONAL = 137,                        // "RATIONAL"
        S_PRIMCHAR = 138,                        // "PRIMCHAR"
        S_PRIMSTRING = 139,                      // "PRIMSTRING"
        S_PRIMINTEGER = 140,                     // "PRIMINTEGER"
        S_PRINTWORD = 141,                       // "PRIMWORD"
        S_PRIMFLOAT = 142,                       // "PRIMFLOAT"
        S_PRIMDOUBLE = 143,                      // "PRIMDOUBLE"
        S_144_ = 144,                            // ','
        S_145_SPECIALISE_ = 145,                 // "{-# SPECIALISE"
        S_146_SPECIALISE_INLINE_ = 146,          // "{-# SPECIALISE_INLINE"
        S_YYACCEPT = 147,                        // $accept
        S_unit = 148,                            // unit
        S_module = 149,                          // module
        S_missing_module_keyword = 150,          // missing_module_keyword
        S_maybemodwarning = 151,                 // maybemodwarning
        S_body = 152,                            // body
        S_body2 = 153,                           // body2
        S_top = 154,                             // top
        S_top1 = 155,                            // top1
        S_maybeexports = 156,                    // maybeexports
        S_exportlist = 157,                      // exportlist
        S_exportlist1 = 158,                     // exportlist1
        S_export = 159,                          // export
        S_export_subspec = 160,                  // export_subspec
        S_qcnames = 161,                         // qcnames
        S_qcnames1 = 162,                        // qcnames1
        S_qcname = 163,                          // qcname
        S_semis1 = 164,                          // semis1
        S_semis = 165,                           // semis
        S_importdecls = 166,                     // importdecls
        S_importdecls_semi = 167,                // importdecls_semi
        S_importdecl = 168,                      // importdecl
        S_optqualified = 169,                    // optqualified
        S_maybeas = 170,                         // maybeas
        S_maybeimpspec = 171,                    // maybeimpspec
        S_impspec = 172,                         // impspec
        S_importlist = 173,                      // importlist
        S_importlist1 = 174,                     // importlist1
        S_import = 175,                          // import
        S_prec = 176,                            // prec
        S_infix = 177,                           // infix
        S_ops = 178,                             // ops
        S_topdecls = 179,                        // topdecls
        S_topdecls_semi = 180,                   // topdecls_semi
        S_topdecl = 181,                         // topdecl
        S_call_conv = 182,                       // call_conv
        S_cl_decl = 183,                         // cl_decl
        S_ty_decl = 184,                         // ty_decl
        S_standalone_kind_sig = 185,             // standalone_kind_sig
        S_sks_vars = 186,                        // sks_vars
        S_inst_decl = 187,                       // inst_decl
        S_overlap_pragma = 188,                  // overlap_pragma
        S_deriv_strategy_no_via = 189,           // deriv_strategy_no_via
        S_deriv_strategy_via = 190,              // deriv_strategy_via
        S_stand_alone_deriving = 191,            // stand_alone_deriving
        S_opt_injective_info = 192,              // opt_injective_info
        S_injectivity_cond = 193,                // injectivity_cond
        S_inj_varids = 194,                      // inj_varids
        S_where_type_family = 195,               // where_type_family
        S_ty_fam_inst_eqn_list = 196,            // ty_fam_inst_eqn_list
        S_ty_fam_inst_eqns = 197,                // ty_fam_inst_eqns
        S_ty_fam_inst_eqn = 198,                 // ty_fam_inst_eqn
        S_at_decl_cls = 199,                     // at_decl_cls
        S_opt_family = 200,                      // opt_family
        S_opt_instance = 201,                    // opt_instance
        S_at_decl_inst = 202,                    // at_decl_inst
        S_data_or_newtype = 203,                 // data_or_newtype
        S_opt_class = 204,                       // opt_class
        S_opt_kind_sig = 205,                    // opt_kind_sig
        S_opt_datafam_kind_sig = 206,            // opt_datafam_kind_sig
        S_opt_tyfam_kind_sig = 207,              // opt_tyfam_kind_sig
        S_opt_at_kind_inj_sig = 208,             // opt_at_kind_inj_sig
        S_tycl_hdr = 209,                        // tycl_hdr
        S_datafam_inst_hdr = 210,                // datafam_inst_hdr
        S_capi_ctype = 211,                      // capi_ctype
        S_maybe_roles = 212,                     // maybe_roles
        S_roles = 213,                           // roles
        S_role = 214,                            // role
        S_decl_cls = 215,                        // decl_cls
        S_decls_cls = 216,                       // decls_cls
        S_decllist_cls = 217,                    // decllist_cls
        S_where_cls = 218,                       // where_cls
        S_decl_inst = 219,                       // decl_inst
        S_decls_inst = 220,                      // decls_inst
        S_decllist_inst = 221,                   // decllist_inst
        S_where_inst = 222,                      // where_inst
        S_decls = 223,                           // decls
        S_decllist = 224,                        // decllist
        S_binds = 225,                           // binds
        S_wherebinds = 226,                      // wherebinds
        S_strings = 227,                         // strings
        S_stringlist = 228,                      // stringlist
        S_opt_tyconsig = 229,                    // opt_tyconsig
        S_sigtype = 230,                         // sigtype
        S_sigtypedoc = 231,                      // sigtypedoc
        S_sig_vars = 232,                        // sig_vars
        S_sigtypes1 = 233,                       // sigtypes1
        S_ktype = 234,                           // ktype
        S_ctype = 235,                           // ctype
        S_ctypedoc = 236,                        // ctypedoc
        S_context = 237,                         // context
        S_context_no_ops = 238,                  // context_no_ops
        S_type = 239,                            // type
        S_typedoc = 240,                         // typedoc
        S_btype = 241,                           // btype
        S_infixtype = 242,                       // infixtype
        S_btype_no_ops = 243,                    // btype_no_ops
        S_ftype = 244,                           // ftype
        S_tyarg = 245,                           // tyarg
        S_tyop = 246,                            // tyop
        S_atype_docs = 247,                      // atype_docs
        S_atype = 248,                           // atype
        S_inst_type = 249,                       // inst_type
        S_deriv_types = 250,                     // deriv_types
        S_comma_types0 = 251,                    // comma_types0
        S_comma_types1 = 252,                    // comma_types1
        S_tv_bndrs = 253,                        // tv_bndrs
        S_tv_bndr = 254,                         // tv_bndr
        S_tv_bndr_no_braces = 255,               // tv_bndr_no_braces
        S_fds = 256,                             // fds
        S_fds1 = 257,                            // fds1
        S_fd = 258,                              // fd
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_literal: // literal
        value.move< Hs::Exp > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_sigdecl: // sigdecl
        value.move< Hs::LDecl > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_apat: // apat
        value.move< Hs::LExp > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_qual: // qual
        value.move< Hs::LStmt > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_altslist: // altslist
        value.move< Hs::ParsedAlts > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_alt: // alt
        value.move< Located<Hs::ParsedAlt> > (std::move (that.value));
        break;

      case symbol_kind::S_role: // role
        value.move< Located<std::optional<Role>> > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_apats1: // apats1
        value.move< std::vector<Hs::LExp> > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_stmts: // stmts
        value.move< std::vector<Hs::LStmt> > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_alts1: // alts1
        value.move< std::vector<Located<Hs::ParsedAlt>> > (std::move (that.value));
        break;

      case symbol_kind::S_maybe_roles: // maybe_roles
      case symbol_kind::S_roles: // roles
        value.move< std::vector<Located<std::optional<Role>>> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Hs::Exp&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Hs::Exp& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Hs::LDecl&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Hs::LDecl& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Hs::LExp&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Hs::LExp& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Hs::LStmt&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Hs::LStmt& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Hs::ParsedAlts&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Hs::ParsedAlts& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Located<Hs::ParsedAlt>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Located<Hs::ParsedAlt>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Located<std::optional<Role>>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Located<std::optional<Role>>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<Hs::LExp>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<Hs::LExp>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<Hs::LStmt>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<Hs::LStmt>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<Located<Hs::ParsedAlt>>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<Located<Hs::ParsedAlt>>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<Located<std::optional<Role>>>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<Located<std::optional<Role>>>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_decl: // decl
      case symbol_kind::S_sigdecl: // sigdecl
        value.template destroy< Hs::LDecl > ();
        break;
      ...
      case symbol_kind::S_bindpat: // bindpat
      case symbol_kind::S_apat: // apat
        value.template destroy< Hs::LExp > ();
        break;
      ...
      case symbol_kind::S_stmt: // stmt
      case symbol_kind::S_qual: // qual
        value.template destroy< Hs::LStmt > ();
        break;
      ...
      case symbol_kind::S_altslist: // altslist
        value.template destroy< Hs::ParsedAlts > ();
        break;
      ...
      case symbol_kind::S_alt: // alt
        value.template destroy< Located<Hs::ParsedAlt> > ();
        break;

      case symbol_kind::S_role: // role
        value.template destroy< Located<std::optional<Role>> > ();
        break;
      ...
      case symbol_kind::S_lexps: // lexps
      case symbol_kind::S_apats1: // apats1
        value.template destroy< std::vector<Hs::LExp> > ();
        break;
      ...
      case symbol_kind::S_guardquals1: // guardquals1
      case symbol_kind::S_stmts: // stmts
        value.template destroy< std::vector<Hs::LStmt> > ();
        break;
      ...
      case symbol_kind::S_alts: // alts
      case symbol_kind::S_alts1: // alts1
        value.template destroy< std::vector<Located<Hs::ParsedAlt>> > ();
        break;

      case symbol_kind::S_maybe_roles: // maybe_roles
      case symbol_kind::S_roles: // roles
        value.template destroy< std::vector<Located<std::optional<Role>>> > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_TRCALL (location_type l)
      {
        return symbol_type (token::TOK_TRCALL, std::move (l));
      }
#else
      static
      symbol_type
      make_TRCALL (const location_type& l)
      {
        return symbol_type (token::TOK_TRCALL, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_ECALL (location_type l)
      {
        return symbol_type (token::TOK_ECALL, std::move (l));
      }
#else
      static
      symbol_type
      make_ECALL (const location_type& l)
      {
        return symbol_type (token::TOK_ECALL, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
  ...
  private:
    ...
    enum
    {
      yylast_ = 5792,     ///< Last index in yytable_.
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_qopm: // qopm
      case symbol_kind::S_literal: // literal
        value.copy< Hs::Exp > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_decl: // decl
      case symbol_kind::S_sigdecl: // sigdecl
        value.copy< Hs::LDecl > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_bindpat: // bindpat
      case symbol_kind::S_apat: // apat
        value.copy< Hs::LExp > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_stmt: // stmt
      case symbol_kind::S_qual: // qual
        value.copy< Hs::LStmt > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_altslist: // altslist
        value.copy< Hs::ParsedAlts > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_alt: // alt
        value.copy< Located<Hs::ParsedAlt> > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_role: // role
        value.copy< Located<std::optional<Role>> > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_lexps: // lexps
      case symbol_kind::S_apats1: // apats1
        value.copy< std::vector<Hs::LExp> > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_guardquals1: // guardquals1
      case symbol_kind::S_stmts: // stmts
        value.copy< std::vector<Hs::LStmt> > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_alts: // alts
      case symbol_kind::S_alts1: // alts1
        value.copy< std::vector<Located<Hs::ParsedAlt>> > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_maybe_roles: // maybe_roles
      case symbol_kind::S_roles: // roles
        value.copy< std::vector<Located<std::optional<Role>>> > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_qopm: // qopm
      case symbol_kind::S_literal: // literal
        value.move< Hs::Exp > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_decl: // decl
      case symbol_kind::S_sigdecl: // sigdecl
        value.move< Hs::LDecl > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_bindpat: // bindpat
      case symbol_kind::S_apat: // apat
        value.move< Hs::LExp > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_stmt: // stmt
      case symbol_kind::S_qual: // qual
        value.move< Hs::LStmt > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_altslist: // altslist
        value.move< Hs::ParsedAlts > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_alt: // alt
        value.move< Located<Hs::ParsedAlt> > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_role: // role
        value.move< Located<std::optional<Role>> > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_lexps: // lexps
      case symbol_kind::S_apats1: // apats1
        value.move< std::vector<Hs::LExp> > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_guardquals1: // guardquals1
      case symbol_kind::S_stmts: // stmts
        value.move< std::vector<Hs::LStmt> > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_alts: // alts
      case symbol_kind::S_alts1: // alts1
        value.move< std::vector<Located<Hs::ParsedAlt>> > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_maybe_roles: // maybe_roles
      case symbol_kind::S_roles: // roles
        value.move< std::vector<Located<std::optional<Role>>> > (YY_MOVE (s.value));
        break;
