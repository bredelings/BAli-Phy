...
equations convertible_to(ptree& model, const type_t& t1, type_t t2)
{
	...
	if (E)
	{
	    ...
	    result.children().push_back({"x",model});
	    model = result;
	}
    }
	...
	if (head3 == "CTMC")
	{
		...
		result.children().push_back({"submodel",model});
		model = result;
	    }
	}
	else
	{
		...
		result.children().push_back({"pairs",model});
		model = result;
	    }
	}
    }
...
TypecheckingState::typecheck_and_annotate_var(const ptree& required_type, const ptree& model) const
{
    ...
    model2.value = name;
    int arity = 0;
    for(auto& [arg_name, arg]: model.children())
    {
	...
	if (not eqs)
	{
		...
		throw myexception()<<"Supplying "<<model.children().size()<<" arguments to function '"<<name<<"', but it only takes "<<arity<<"!";
	}

	// 2c. Typecheck the argument
	auto arg2 = typecheck_and_annotate(a, arg);

	model2.children().push_back({"",arg2});
...
ptree TypecheckingState::typecheck_and_annotate_function(const ptree& required_type, const ptree& model) const
{
    ...
    for(const auto& [arg_name,_]: model.children())
    {
	...
	else if (argument.default_value)
	{
        ...
        if (alphabet_value)
            arg_value2.children().push_back({"alphabet",*alphabet_value});
        arg_value2.children().push_back({"is_default_value",ptree(is_default)});
        if (not is_default)
            add(used_args, get_used_args(arg_value2));

	model2.children().push_back({arg_name, arg_value2});
    }

    model2 = ptree({{"value",model2},{"type",result_type}});
    if (rule.no_log)
	model2.children().push_back({"no_log",ptree(true)});
    if (rule.extract)
	model2.children().push_back({"extract",ptree(*rule.extract)});
