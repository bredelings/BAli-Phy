...
#include "TreeInterface.hh"                          // for TreeInterface
#include "alignment/alignment.hh"                    // for alignment
#include "computation/object.hh"                     // for Object, intrusive...
#include "computation/param.hh"                      // for param
#include "sequence/sequence.hh"                      // for sequence
#include "tools/partition.hh"                        // for Partition
#include "tree/sequencetree.hh"                      // for SequenceTree
#include "tree/tree.hh"                              // for tree_edge
#include "util/assert.hh"                           // for assert
#include "util/math/log-double.hh"                   // for log_double_t
#include "util/dense-matrix.hh"                      // for DenseMatrix
...
public:
    ...
    object_ptr<const R::RVector> transition_P(int b) const;   // used only to force transition_Ps in sample-topology-SPR.cc [ SPR_search_attachment_points ]
  
    R::Exp get_node_CLV(int i) const;   // used in substitution.cc [sample_subst_history], parsimony.cc [peel_muts_leaf_branch, accumulate_root_leaf]
    ...
    object_ptr<const Box<DenseMatrix<double>>> WeightedFrequencyMatrix(int n) const;     // used in sample-alignment.cc, sample-cube.cc, sample-tri.cc
    ...
    R::Exp get_pairwise_alignment_(int b) const;
...
public:
    ...
    bool variable_alignment_ = false;
