...
class Likelihood_Cache_Branch: public Object
{
    ...
    std::optional<ProbDensity> other_subst_;
public:
    Likelihood_Cache_Branch* clone() const {std::abort();}

    void init_other_subst(ProbDensity p=1) {assert(not other_subst_); other_subst_ = p;}
          ProbDensity& other_subst() {return other_subst_.value();}
    const ProbDensity& other_subst() const {return other_subst_.value();}
    std::function<ProbDensity(const DenseMatrix<double>&)> other_subst_f;

    boost::dynamic_bitset<> bits;
    std::optional<DenseMatrix<double>> away_from_root_WF;
    ...
    void set(int c, const DenseMatrix<double>& M)
    {
	double* __restrict__ m1 = data + matrix_size()*c;
	const double* __restrict__ m2 = M.data();
	for(int i=0;i<matrix_size();i++)
	    m1[i] = m2[i];
    }
    ...
    void get(int c, DenseMatrix<double>& M) const
    {
	double* __restrict__ m1 = M.data();
	const double* __restrict__ m2 = data + matrix_size()*c;
	for(int i=0;i<matrix_size();i++)
	    m1[i] = m2[i];
    }

    void mul(int c, const DenseMatrix<double>& M)
    {
	double* __restrict__ m1 = data + matrix_size()*c;
	const double* __restrict__ m2 = M.data();
	for(int i=0;i<matrix_size();i++)
    ...
    double dot(int c, const DenseMatrix<double>& M) const
    {
