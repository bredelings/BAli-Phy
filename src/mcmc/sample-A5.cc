...
sample_A5_base(mutable_data_partition P, const vector<HMM::bitmask_t>& a123456, const A5::hmm_order& order, const A5::hmm_order& order0)
{
    ...
    {
        if (log_verbose > 0)
            std::cerr<<"sample_A5_base( ): sampling probability is "<<sampling_pr<<"!"<<std::endl;
        return {Matrices, unavailable};
    }
...
Availability<log_double_t> sample_A5_ratio(vector<Parameters>& p, const vector<A5::hmm_order>& order,
...
///(a[0],p[0]) is the point from which the proposal originates, and must be valid.
optional<int> sample_A5_multi(vector<Parameters>& p,const vector<A5::hmm_order>& order_,
			      const vector<log_double_t>& rho_)
{
    for(int i=1;i<p.size();i++)
	for(int j=0;j<p[0].n_data_partitions();j++)
...
void sample_A5(Parameters& P,int b) 
{
    ...
    auto choice = sample_A5_multi(p,order,rho);
