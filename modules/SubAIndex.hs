...
builtin merge_suba_indices 4 "merge_suba_indices" "SubAIndex";

subA_index_leaf t a     = let {indices = mkArray (2*numBranches t) index;
                               ranks = mkArray (2*numBranches t) (rank t);

                               rank t b = if (is_leaf_node t (sourceNode t b))
                                          then sourceNode t b
                                          else foldl1 min $ map (ranks!) $ edgesBeforeEdge t b;
                                              
                               index b = let {s = sourceNode t b} in
                                         if (is_leaf_node t s) then
                                             get_column_index_list_for_characters a (sourceNode t b)
                                         else
                                             case (edgesBeforeEdge t b) of {[b1,b2] -> merge_suba_indices (indices!b1) (ranks!b1) (indices!b2) (ranks!b2)}
                              }
                          in indices;
