...
-- Check densities, moments, endpoints, and saturation beyond Int range; this guards against
-- standard-library rejection loops and is obsolete with an unbounded result type.
main = do
    let dist = negativeBinomial 2 0.25
        ...
        zeroSuccessProbability = negativeBinomial 2 0.0
    sample1 <- sampleIO successCertain
    sample0 <- sampleIO noSuccessesNeeded
    sampleImpossible <- sampleIO zeroSuccessProbability
    saturatedSample <- sampleIO (negativeBinomial 2 1.0e-300)
    print (near (probability 1) 0.09375
        ...
        && pdf zeroSuccessProbability 0 == 0
        && near (mean dist) 6.0
        && near (variance dist) 24.0
        && sample1 == 0
        && sample0 == 0
        && sampleImpossible == 2147483647
        && saturatedSample == 2147483647)
