...
def run_test_cmd(test_dir, data_dir, cmd):
    ...
    obt_outf = os.path.join(test_dir, 'rb-obtained-output')
    obt_errf = os.path.join(test_dir, 'rb-obtained-error')
    obt_exitf = os.path.join(test_dir, 'rb-obtained-exit')
    obt_likef = os.path.join(test_dir, 'rb-likelihood')
...
def check_likelihood(test_dir):
    ...
    outputf   = os.path.join(test_dir, 'rb-obtained-output')
...
def check_test_output(test_dir, name):
    ...
    if not check_expected(test_dir, 'rb-obtained-output', 'rb-output'):
        failures.append('output')
    if not check_expected(test_dir, 'rb-obtained-error', 'rb-error'):
        failures.append('error')
    if not check_expected(test_dir, 'rb-obtained-exit', 'rb-exit'):
    ...
    if exit_test_failed:
        expected_exitf = os.path.join(test_dir, 'rb-exit')
        expected_exit = codecs.open(expected_exitf, 'r').read()
        if expected_exit.rstrip() == "0":
            obtained_errorf = os.path.join(test_dir, 'rb-obtained-error')
...
def perform_test(data_dir, top_test_dir,test_subdir,cmd):
    ...
    obt_errf = os.path.join(test_dir, 'rb-obtained-error')
    obt_exitf = os.path.join(test_dir, 'rb-obtained-exit')
    obt_likef = os.path.join(test_dir, 'rb-obtained-likelihood')
